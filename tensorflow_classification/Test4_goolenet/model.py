...
def GoogLeNet(im_height=224, im_width=224, class_num=1000, aux_logits=False):
    ...
    # (None, 112, 112, 64)    224/2 = 112；  
    # *  观察列表，变为一半，所以是same模式   same模式，N = W/Stride，直接除以步长即可；  
    ...
    if aux_logits:
        model = models.Model(inputs=input_image, outputs=[aux1, aux2, aux3])   # 如果有辅助分类器，就有3个输出
    else:
...
class Inception(layers.Layer):
    def __init__(self, ch1x1, ch3x3red, ch3x3, ch5x5red, ch5x5, pool_proj, **kwargs):
        ...
        self.branch1 = layers.Conv2D(ch1x1, kernel_size=1, activation="relu")     # 卷积核个数是ch1x1

        self.branch2 = Sequential([                                                       # 使用 Sequential 将两个卷积层连在一起
    ...
    def call(self, inputs, **kwargs):                                   # ! call函数就是正向传播的函数
        branch1 = self.branch1(inputs)
        branch2 = self.branch2(inputs)
        branch3 = self.branch3(inputs)
        branch4 = self.branch4(inputs)
        outputs = layers.concatenate([branch1, branch2, branch3, branch4])  #  layers.concatenate 在深度方向进行拼接
        return outputs


class InceptionAux(layers.Layer):                               # 辅助分类器
